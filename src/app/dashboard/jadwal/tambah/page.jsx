...
export default function TambahJadwalPage() {
	...
	const fetchDrivers = useCallback(async () => {
		try {
			...
			setDrivers(
				data.data.filter((u) => u.role === "driver" && u.status === "active")
			);
		} catch (error) {
	...
	const calculateTotals = useCallback(() => {
		...
		selected.forEach((order) => {
			order.order_detail.forEach((item) => {
				totalWeight += item.produk.berat * item.jumlah;
				totalVolume +=
					item.produk.panjang *
					item.produk.lebar *
					item.produk.tinggi *
					item.jumlah; // Correct volume calculation
			});
		});
								...
								{selectedOrders().length > 0 ? (
									<div className="space-y-3">
										...
										<div className="flex justify-between items-center p-3 bg-gray-100 rounded-md">
											<div>
												<p className="text-sm font-medium">Total Berat:</p>
												<p className="text-sm">
													{calculateTotals().totalWeight} kg
												</p>
											</div>
											<div>
												<p className="text-sm font-medium">Total Volume:</p>
												<p className="text-sm">
													{calculateTotals().totalVolume} m³
												</p>
											</div>
										</div>
